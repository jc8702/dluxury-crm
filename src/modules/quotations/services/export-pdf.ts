...
export const exportBudgetToPDF = (quotation: any) => {
  if (!quotation) return;
  ...
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(`ORÇAMENTO: #${quotation.numeroOrcamento}`, pageWidth - 20, 25, { align: 'right' });

  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(format(new Date(), "dd 'de' MMMM 'de' yyyy", { locale: ptBR }), pageWidth - 20, 33, {
    align: 'right',
  });
  ...
  const clienteNome = quotation.cliente?.nome || 'NÃO INFORMADO';
  const clienteLocal = quotation.cliente
    ? `${quotation.cliente.cidade || ''} - ${quotation.cliente.uf || ''}`
    : '---';
  ...
  doc.setTextColor(15, 15, 15);
  doc.text(`${quotation.validadeDias} dias`, 60, 84);

  // 3. ITEMS TABLE
  const tableBody = (quotation.itens || []).map((item: any) => [
    {
    ...
    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(
      Number(item.precoVendaUnitario || 0),
    ),
    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(
      Number(item.precoVendaUnitario || 0) * Number(item.quantidade),
    ),
  ]);
  ...
  doc.setFontSize(18);
  doc.setFont('helvetica', 'bold');
  const totalVendaStr = new Intl.NumberFormat('pt-BR', {
    style: 'currency',
    currency: 'BRL',
  }).format(Number(quotation.valorTotalVenda));
  ...
  const condicoes = [
    `- Proposta válida por ${quotation.validadeDias} dias.`,
    `- Taxa financeira inclusa: ${quotation.taxaFinanceiraPercentual}%.`,
  ...
  doc.text(
    "D'LUXURY AMBIENTES - SISTEMA DE GESTÃO INDUSTRIAL ERP",
    pageWidth / 2,
    pageHeight - 15,
    { align: 'center' },
  );
  doc.text(
    'Documento gerado eletronicamente. Proposta sujeita a análise técnica final.',
    pageWidth / 2,
    pageHeight - 11,
    { align: 'center' },
  );

  doc.save(`Proposta_DLuxury_${quotation.numeroOrcamento}.pdf`);
};
