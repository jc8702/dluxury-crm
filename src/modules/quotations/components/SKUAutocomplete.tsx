        ...
        <div className={`relative ${className}`} ref={containerRef}>
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full bg-background border border-border text-foreground pl-10 pr-4 py-2 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none placeholder:text-muted-foreground/60"
                />
                {loading && (
                    ...
                    {results.length === 0 && !loading ? (
                        ...
                        <div className="py-2">
                            {results.map((sku) => (
                                ...
                                >
                                    <div className={`mt-1 p-1.5 rounded-md ${sku.tipo === 'INDUSTRIAL' ? 'bg-primary/10 text-primary' : 'bg-info/10 text-info'}`}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-center gap-2">
                                            ...
                                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${sku.tipo === 'INDUSTRIAL' ? 'bg-primary/10 text-primary' : 'bg-info/10 text-info'}`}>
                                                {sku.tipo}
                                            </span>
                                        </div>
