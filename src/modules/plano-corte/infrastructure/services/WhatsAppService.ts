...
export class WhatsAppService {
   ...
   */
  static async notificarProducaoIniciada(
    clienteNome: string,
    clienteTelefone: string,
    projetoNome: string,
  ) {
    const _mensagem = `Olá ${clienteNome}! 👋 Aqui é da D'Luxury. 
...
Em breve seu sonho estará pronto.`;
    ...
    /* console.log(`[WA] Enviando para ${clienteTelefone}: ${mensagem}`) */ // Simulação de delay de rede
   ...
   */
  static async notificarProjetoPronto(
    clienteNome: string,
    clienteTelefone: string,
    projetoNome: string,
  ) {
    const _mensagem = `Boas notícias, ${clienteNome}! 🎉 
...
Nossa equipe entrará em contato para agendar a entrega/montagem.`;
    ...
    /* console.log(`[WA] Enviando para ${clienteTelefone}: ${mensagem}`) */ return new Promise(
