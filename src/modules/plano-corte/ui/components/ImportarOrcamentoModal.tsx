...
export function ImportarOrcamentoModal({ onImportar, onFechar }: ImportarOrcamentoModalProps) {
  ...
  const handleSelecionarOrcamento = async (orcamentoId: string, numeroOrcamento: string) => {
    ...
    try {
      ...
      itensMdf.forEach((item: any) => {
        ...
        const materialClean = materialRaw.toUpperCase().replace(/\s*\d+\s*(?:MM|CM).*$/i, '').trim(); // Remove espessura do nome
        ...
        const peca = {
          ...
          nome: item.nomeCustomizado || item.skuDescricao || 'PEÇA ORÇAMENTO',
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onFechar}
      title="Importar Peças do Orçamento"
      size="md"
    >
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-3 custom-scrollbar min-h-[250px] max-h-[450px]">
          {loading || importing ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-muted-foreground opacity-50">
              ...
              <h3 className="text-xs font-bold uppercase tracking-widest">Nenhum orçamento encontrado</h3>
            </div>
          )}
        </div>
      </div>
