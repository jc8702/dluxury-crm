...
export const PainelRetalhos: React.FC = () => {
  ...
  const carregarEstoque = async () => {
    ...
    try {
      ...
      let filtrados = data;
      if (filtro.sku_chapa)
        filtrados = filtrados.filter((r) => r.sku_chapa.includes(filtro.sku_chapa!));
      if (filtro.disponivel !== undefined)
        filtrados = filtrados.filter((r) => r.disponivel === filtro.disponivel);
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex justify-between items-start flex-wrap gap-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm font-medium">
            Otimização e reuso de sobras de MDF industrial
          </p>
        </div>
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-20 text-center text-muted-foreground">
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest opacity-50">
                      Sincronizando estoque industrial...
                    </p>
                  </td>
                </tr>
              ) : retalhos.length === 0 ? (
                  ...
                  <tr key={retalho.id} className="group hover:bg-white/[0.02] transition-all">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[10px] text-muted-foreground font-medium uppercase tracking-tight">
                        {retalho.nome_material || 'MDF PADRÃO'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
      ...
      <div className="glass p-4 flex items-center gap-3 rounded-xl border border-warning/20 border-l-4 border-l-warning bg-warning/5">
        ...
        <span className="text-[11px] text-muted-foreground font-semibold leading-tight">
          ...
          Retalhos menores que 300x300mm são automaticamente removidos para otimização de espaço
          físico.
        </span>
      </div>
    </div>
