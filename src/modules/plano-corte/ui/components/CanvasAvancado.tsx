...
export function CanvasAvancado({
    ...
    if (localLayout && localLayout.pecas_posicionadas) {
      localLayout.pecas_posicionadas.forEach((peca) => {
        ...
        desenharPeca(
          ctx,
          peca,
          peca.id === hoveredPeca || isSelected,
          viewport.zoom,
          showMeasurements,
          isCortada,
          executionMode,
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const worldX = (mouseX - viewport.x) / viewport.zoom;
      const worldY = (mouseY - viewport.y) / viewport.zoom;
      const pecaClicada = localLayout?.pecas_posicionadas?.find(
        (p) =>
          worldX >= p.x && worldX <= p.x + p.largura && worldY >= p.y && worldY <= p.y + p.altura,
      );
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
          ...
          return {
            ...
            pecas_posicionadas: prev.pecas_posicionadas.map((p) => {
              ...
              const novoX = Math.max(
                0,
                Math.min(worldX - dragOffset.x, chapaDimensoes.largura - p.largura),
              );
              const novoY = Math.max(
                0,
                Math.min(worldY - dragOffset.y, chapaDimensoes.altura - p.altura),
              );
      ...
      const pecaHover = localLayout?.pecas_posicionadas?.find(
        (p) =>
          worldX >= p.x && worldX <= p.x + p.largura && worldY >= p.y && worldY <= p.y + p.altura,
      );
  ...
  const pecaSelecionadaObj =
    selectedPeca && localLayout
      ? localLayout.pecas_posicionadas?.find((p) => p.id === selectedPeca)
      : null;
    ...
    >
      ...
      {(hoveredPeca || selectedPeca) && (
        <div className="absolute top-6 left-6 w-64 glass-elevated p-4 rounded-xl z-20 animate-fade-in border-l-4 border-l-primary">
          {(() => {
            const peca = localLayout?.pecas_posicionadas?.find(
              (p) => p.id === (selectedPeca || hoveredPeca),
            );
            if (!peca) return null;
                  ...
                  {peca.fio_de_fita && (
                    <div className="pt-2 border-t border-border/50">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {peca.fio_de_fita.topo && (
                          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded border border-red-500/50 text-red-500 bg-red-500/10">
                            TOPO
                          </span>
                        )}
                        {peca.fio_de_fita.baixo && (
                          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded border border-blue-500/50 text-blue-500 bg-blue-500/10">
                            BAIXO
                          </span>
                        )}
                        {peca.fio_de_fita.esquerda && (
                          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded border border-green-500/50 text-green-500 bg-green-500/10">
                            ESQ
                          </span>
                        )}
                        {peca.fio_de_fita.direita && (
                          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded border border-amber-500/50 text-amber-500 bg-amber-500/10">
                            DIR
                          </span>
                        )}
                      </div>
...
function desenharBordaChapa(
  ...
  // Top Right
  ctx.beginPath();
  ctx.moveTo(dimensoes.largura - cornerSize, 0);
  ctx.lineTo(dimensoes.largura, 0);
  ctx.lineTo(dimensoes.largura, cornerSize);
  ctx.stroke();
  // Bottom Left
  ctx.beginPath();
  ctx.moveTo(0, dimensoes.altura - cornerSize);
  ctx.lineTo(0, dimensoes.altura);
  ctx.lineTo(cornerSize, dimensoes.altura);
  ctx.stroke();
  // Bottom Right
  ctx.beginPath();
  ctx.moveTo(dimensoes.largura - cornerSize, dimensoes.altura);
  ctx.lineTo(dimensoes.largura, dimensoes.altura);
  ctx.lineTo(dimensoes.largura, dimensoes.altura - cornerSize);
  ctx.stroke();
