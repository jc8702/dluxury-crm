...
describe('Validação Industrial do Plano de Corte', () => {
  ...
  it('deve otimizar o corte com aproveitamento superior a 80%', () => {
    ...
    /* console.log(`\n--- RESULTADO DA OTIMIZAÇÃO ---`) */ /* console.log(`Aproveitamento: ${resultado.aproveitamento.toFixed(2)}%`) */ /* console.log(`Peças Posicionadas: ${resultado.pecas_posicionadas.length}/${PECAS_TESTE.length}`) */ /* console.log(`Tempo de Cálculo: ${resultado.tempo_calculo_ms}ms`) */ expect(
  ...
  it('deve gerar G-Code válido para a primeira chapa do plano', () => {
    ...
    /* console.log(`\n--- G-CODE GERADO (PREVIEW) ---`) */ /* console.log(gcode.substring(0, 500) + '...') */ expect(
      gcode,
    ).toContain('G21'); // Unidades mm
