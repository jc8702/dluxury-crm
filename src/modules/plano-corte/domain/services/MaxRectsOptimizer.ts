import type { Peca, PecaPosicionada, FreeRectangle, Sobra, ResultadoOtimizacao } from '../types';
...
export class MaxRectsOptimizer {
  ...
  otimizar(pecas: Peca[]): ResultadoOtimizacao {
    ...
    const areaUsada = this.pecasPosicionadas.reduce(
      (acc, p) => acc + (p.largura * p.altura), 
      0
    );
    ...
    return {
      ...
      sobras: this.gerarSobras(),
      aproveitamento: (areaUsada / areaTotal) * 100,
      area_usada: areaUsada,
      area_total: areaTotal,
      algoritmo_usado: 'MaxRects BSSF'
    };
  }

  private gerarSobras(): Sobra[] {
  ...
  private encontrarMelhorPosicao(pecas: Peca[]): { 
    ...
    if (melhorPeca && melhorPosicao) {
      return { peca: melhorPeca, posicao: melhorPosicao };
    }

    return null;
  }

  private dividirFreeRectangles(pecaPos: {
    x: number;
    y: number;
    largura: number;
    altura: number;
    ...
    const pecaComKerf = {
      x: pecaPos.x,
      y: pecaPos.y,
      width: pecaPos.largura + this.kerfMm,
      height: pecaPos.altura + this.kerfMm
    };
    ...
    for (const rect of this.freeRectangles) {
      ...
      if (pecaComKerf.x > rect.x) {
        novosFreeRects.push({
          x: rect.x,
          y: rect.y,
          width: pecaComKerf.x - rect.x,
          height: rect.height
        });
      }

      if (pecaComKerf.x + pecaComKerf.width < rect.x + rect.width) {
        novosFreeRects.push({
          x: pecaComKerf.x + pecaComKerf.width,
          y: rect.y,
          width: rect.x + rect.width - (pecaComKerf.x + pecaComKerf.width),
          height: rect.height
        });
      }

      if (pecaComKerf.y > rect.y) {
        novosFreeRects.push({
          x: rect.x,
          y: rect.y,
          width: rect.width,
          height: pecaComKerf.y - rect.y
        });
      }

      if (pecaComKerf.y + pecaComKerf.height < rect.y + rect.height) {
        novosFreeRects.push({
          x: rect.x,
          y: pecaComKerf.y + pecaComKerf.height,
          width: rect.width,
          height: rect.y + rect.height - (pecaComKerf.y + pecaComKerf.height)
        });
      }
    }
  ...
  private intersecta(a: FreeRectangle, b: FreeRectangle): boolean {
    return !(
      a.x >= b.x + b.width ||
      a.x + a.width <= b.x ||
      a.y >= b.y + b.height ||
