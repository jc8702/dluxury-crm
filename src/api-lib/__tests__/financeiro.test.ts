...
vi.mock('../_db.js', () => {
  ...
  const sqlFn: any = vi.fn(async (...args: any[]) => {
    ...
    if (q.includes('update contas_internas set')) {
      mockSqlStore.contasRetorno = [{ id: '00000000-0000-4000-8000-000000000000', saldo_inicial: 200, saldo_atual: 1100 }];
      return [];
    }
  ...
  sqlFn.begin = vi.fn(async (cb: any) => {
    const txFn = vi.fn(async (...args: any[]) => {
      ...
      if (q.includes('select id, tipo, dia_vencimento, descricao, valor, cliente_id, fornecedor_id, classe_financeira_id, forma_pagamento_id, conta_bancaria_id from contas_recorrentes')) {
        return [{ id: 'cr-1', tipo: 'receber', dia_vencimento: 10, valor: 100, descricao: 'Recorrente Teste', cliente_id: '00000000-0000-4000-8000-000000000000', classe_financeira_id: '00000000-0000-4000-8000-000000000000' }];
      }
...
describe('handleFinanceiro router', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(validateAuth).mockReturnValue({ authorized: true, user: { id: 'u1', tenantId: mockUuid }, error: null });
...
describe('handleContasInternas', () => {
  ...
  it('deve obter extrato da conta (GET /extrato)', async () => {
    _mockSqlStore.contasRetorno = [{ id: 'c1', nome: 'Conta Corrente', saldo_inicial: 500, saldo_atual: 1000 }];
    _mockSqlStore.movimentosRetorno = [
      { id: 'b1', data: '2026-05-01', valor: 200, tipo: 'recebimento', descricao: 'Baixa', origem: 'baixa' },
