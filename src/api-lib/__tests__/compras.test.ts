...
describe('handleCompras', () => {
  ...
  it('deve atualizar pedido e gerar titulos a pagar ao confirmar (PATCH type=pedidos&id=X)', async () => {
    vi.mocked(sql)
      .mockResolvedValueOnce([{ id: '1', numero: 'PC-2026-001', valor_total: 1500, fornecedor_id: 2 }]) // UPDATE do pedido
  ...
  it('deve registrar recebimento de itens e atualizar estoque (POST type=recebimento)', async () => {
    vi.mocked(sql)
      ...
      .mockResolvedValueOnce([{ id: 'item-1', material_id: 'mat-1', preco_unitario: 15, quantidade_pedida: 10, quantidade_recebida: 10 }]) // UPDATE pedido_compra_itens
