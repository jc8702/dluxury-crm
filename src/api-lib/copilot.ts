...
async function generateBOM(payload: any, tenantId: string) {
  const materiais =
    await sql`SELECT id, nome, categoria_id, preco_custo, unidade_uso FROM materiais WHERE ativo = true AND tenant_id = ${tenantId}::uuid`;
  try {
    ...
    logger.warn('Fallback on generateBOM due to error.');
...
async function auditSKU(payload: any, tenantId: string) {
  const existentes =
    await sql`SELECT nome, descricao, categoria_id FROM materiais WHERE ativo = true AND tenant_id = ${tenantId}::uuid`;
  try {
    const { object } = await generateObject({
      model: modelPro,
      schema: z.object({
    ...
    return {
      ...
      recomendacao: 'FALHA NA VALIDAÇÃO IA.',
    };
  }
}
...
async function translateDescription(payload: any) {
  ...
  try {
    ...
    return { original: text, translated: text, lang: targetLang };
  }
}
...
async function generateProposalPDF(payload: any) {
  ...
  try {
    ...
    return { markdown: 'Erro ao gerar PDF', generated_at: new Date().toISOString() };
  }
}
...
async function forecastDemand(_payload: any, tenantId: string) {
  ...
  try {
    ...
    return { previsao_proximo_mes: 0, tendencia: 'estavel', sazonalidade: [], recomendacoes: [] };
  }
}
...
async function getRawLLMIntent(message: string, history: any[] = []): Promise<string> {
  const context = history
    .map((h) => `${h.role === 'user' ? 'USUÁRIO' : 'COPILOTO'}: ${h.content}`)
    .join('\n');
...
async function parseIntent(message: string, history: any[] = []): Promise<Intent> {
  try {
    ...
    if (!jsonMatch) {
      logger.error('JSON não encontrado na resposta bruta:', text);
      return { type: 'UNKNOWN', entities: {} };
    }
    const parsed = JSON.parse(jsonMatch[0]);
    return sanitizeIntent(parsed);
  } catch (error) {
    logger.error('Erro fatal no parseIntent:', error);
    return { type: 'UNKNOWN', entities: {} };
  }
}
...
const SKUService = {
  async checkDuplicity(descricao: string, tenantId: string) {
    const r =
      await sql`SELECT sku, nome FROM materiais WHERE nome ILIKE ${'%' + descricao + '%'} AND tenant_id = ${tenantId}::uuid LIMIT 1`;
    return r.length > 0 ? r[0] : null;
  },
  async create(data: Entities, tenantId: string) {
    let categoryId = 'OUT';
    const famLow = (data.familia || '').toLowerCase();
    if (famLow.includes('chapa') || famLow.includes('mdf') || famLow.includes('mdp'))
      categoryId = 'CHP';
    else if (famLow.includes('fita') || famLow.includes('borda') || famLow.includes('pvc'))
      categoryId = 'BRD';
    else if (
      famLow.includes('dobradiça') ||
      famLow.includes('ferragem') ||
      famLow.includes('puxador')
    )
      categoryId = 'FRG';
    else if (famLow.includes('parafuso') || famLow.includes('bucha') || famLow.includes('fix'))
      categoryId = 'FIX';
    else categoryId = (data.familia || 'GEN').substring(0, 3).toUpperCase();

    const lastSkuQuery =
      await sql`SELECT sku FROM materiais WHERE categoria_id = ${categoryId} AND tenant_id = ${tenantId}::uuid ORDER BY sku DESC LIMIT 1`;
    let proximoSku = `${categoryId}-0001`;
    if (lastSkuQuery.length > 0 && lastSkuQuery[0].sku) {
      const match = lastSkuQuery[0].sku.match(/\d+/);
      if (match) {
        proximoSku = `${categoryId}-${(parseInt(match[0], 10) + 1).toString().padStart(4, '0')}`;
      }
    }
  ...
  async getLast(tenantId: string) {
    const r =
      await sql`SELECT sku as "skuId", nome as descricao FROM materiais WHERE tenant_id = ${tenantId}::uuid ORDER BY id DESC LIMIT 1`;
    return r.length > 0 ? r[0] : null;
  },
  async search(filtro: Entities, tenantId: string) {
    const searchString = '%' + (filtro.descricao || filtro.familia || '') + '%';
    const r =
      await sql`SELECT sku as "skuId", nome as descricao FROM materiais WHERE nome ILIKE ${searchString} AND tenant_id = ${tenantId}::uuid LIMIT 5`;
    return r;
  },
  async listByFamilia(familia: string, tenantId: string) {
    const famStr = familia.substring(0, 3).toUpperCase();
    const r =
      await sql`SELECT sku as "skuId", nome as descricao FROM materiais WHERE categoria_id = ${famStr} AND tenant_id = ${tenantId}::uuid LIMIT 5`;
    return r;
  },
};
...
async function handleConfirmAction(history: any[], tenantId: string) {
  ...
  if (!lastAiMessage)
    return { message: 'O que você deseja confirmar? Não identifiquei uma sugestão pendente.' };

  const content = lastAiMessage.content;
  const isBOM =
    content.includes('Engenharia') ||
    content.includes('📐') ||
    content.includes('Projeto') ||
    content.includes('BOM');
  ...
  if (isSKU) {
    ...
    return {
      message: `✅ Perfeito! Item cadastrado com sucesso.\n\n**SKU:** ${sku.skuId}\n**Descrição:** ${sku.descricao}\n\n[EVENT_EMIT_SKU_CRIADO]`,
    };
  }

  if (isBOM) {
    const userProjectMsg = [...history]
      .reverse()
      .find((m, i, arr) => m.role === 'user' && i > arr.indexOf(lastAiMessage));
...
async function handleAnalyzeStock(tenantId: string) {
  const criticos =
    await sql`SELECT sku, nome, estoque_atual, estoque_minimo FROM materiais WHERE estoque_atual <= estoque_minimo AND ativo = true AND tenant_id = ${tenantId}::uuid LIMIT 5`;

  if (criticos.length === 0)
    return { message: '✅ Estoque saudável! Nenhum item crítico identificado no momento.' };
...
async function processUserMessage(message: string, history: any[] = [], tenantId: string) {
  try {
    ...
    switch (intent.type) {
      ...
      default: {
        const vendas =
          await sql`SELECT SUM(valor_total) as total FROM titulos_receber WHERE status = 'recebido' AND created_at > NOW() - INTERVAL '30 days' AND tenant_id = ${tenantId}::uuid`;
        const estoque =
          await sql`SELECT nome, estoque_atual FROM materiais WHERE ativo = true AND tenant_id = ${tenantId}::uuid ORDER BY estoque_atual ASC LIMIT 5`;
        const clientes =
          await sql`SELECT nome FROM clientes WHERE tenant_id = ${tenantId}::uuid ORDER BY nome LIMIT 5`;
        const ops =
          await sql`SELECT op_id, produto, status FROM ordens_producao WHERE tenant_id = ${tenantId}::uuid ORDER BY created_at DESC LIMIT 5`;
    ...
    logger.error('Erro fatal no ProcessUserMessage:', error);
    return {
      message: 'Desculpe, tive um problema técnico ao processar sua solicitação. Pode repetir?',
    };
  }
}
