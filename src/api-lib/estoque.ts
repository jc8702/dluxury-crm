  ...
  try {
    ...
    if (type === 'movimentacoes') {
      ...
      if (method === 'POST') {
        ...
        const mRes =
          await sql`SELECT estoque_atual, fator_conversao, preco_custo, nome FROM materiais WHERE id = ${material_id} AND tenant_id = ${tenantId}`;
    ...
    if (type === 'fornecedores') {
      if (method === 'GET') {
        const result = id
          ? (
              await sql`SELECT id, nome, cnpj, contato, telefone, email, cidade, estado, observacoes, ativo, created_at, updated_at FROM fornecedores WHERE id = ${id} AND tenant_id = ${tenantId}`
            )[0]
      ...
      if (method === 'POST') {
        const f = req.body;
        const r =
          await sql`INSERT INTO fornecedores (nome, cnpj, contato, telefone, email, cidade, estado, observacoes, tenant_id) VALUES (${f.nome}, ${f.cnpj}, ${f.contato}, ${f.telefone}, ${f.email}, ${f.cidade}, ${f.estado}, ${f.observacoes}, ${tenantId}) RETURNING *`;
        return res.status(201).json({ success: true, data: r[0] });
      }
      if (method === 'PATCH') {
        const f = req.body;
        const r =
          await sql`UPDATE fornecedores SET nome = ${f.nome}, cnpj = ${f.cnpj}, contato = ${f.contato}, telefone = ${f.telefone}, email = ${f.email}, cidade = ${f.cidade}, estado = ${f.estado}, observacoes = ${f.observacoes}, updated_at = CURRENT_TIMESTAMP WHERE id = ${id} AND tenant_id = ${tenantId} RETURNING *`;
        if (!r[0])
    ...
    if (type === 'categories') {
      if (method === 'GET') {
        const result =
          await sql`SELECT id, nome, ativo, created_at FROM erp_categories WHERE tenant_id = ${tenantId} ORDER BY nome ASC`;
        return res.status(200).json({ success: true, data: result });
      }
      if (method === 'POST') {
        const r =
          await sql`INSERT INTO erp_categories (id, nome, ativo, tenant_id) VALUES (${req.body.slug?.toUpperCase() || req.body.nome.substring(0, 3).toUpperCase()}, ${req.body.nome}, true, ${tenantId}) RETURNING *`;
        return res.status(201).json({ success: true, data: r[0] });
      }
    }
    ...
    if (method === 'GET') {
      if (id) {
        const mat =
          await sql`SELECT m.*, c.nome as categoria_nome FROM materiais m LEFT JOIN erp_categories c ON m.categoria_id = c.id WHERE m.id = ${id} AND m.tenant_id = ${tenantId} AND (c.tenant_id IS NULL OR c.tenant_id = ${tenantId})`;
        const movs =
          await sql`SELECT id, material_id, tipo, quantidade, motivo, projeto_id, quotation_id, preco_unitario, valor_total, estoque_antes, estoque_depois, created_by, created_at, nota_fiscal FROM movimentacoes_estoque WHERE material_id = ${id} AND tenant_id = ${tenantId} ORDER BY created_at DESC LIMIT 50`;
        return res.status(200).json({ success: true, data: { ...mat[0], movements: movs } });
      }
      const { q } = req.query;
      if (q) {
        const mats =
          await sql`SELECT m.id, m.sku, m.nome, c.nome as categoria_nome, m.preco_custo FROM materiais m LEFT JOIN erp_categories c ON m.categoria_id = c.id WHERE m.ativo = true AND m.tenant_id = ${tenantId} AND (c.tenant_id IS NULL OR c.tenant_id = ${tenantId}) AND (m.sku ILIKE ${'%' + q + '%'} OR m.nome ILIKE ${'%' + q + '%'}) LIMIT 10`;
        const mods =
          await sql`SELECT id, codigo_modelo as sku, nome, 'Módulo de Engenharia' as categoria_nome, 0 as preco_custo, largura_padrao, altura_padrao, profundidade_padrao, horas_mo_padrao, valor_hora_padrao, preco_material_m3_padrao FROM erp_product_bom WHERE (codigo_modelo ILIKE ${'%' + q + '%'} OR nome ILIKE ${'%' + q + '%'}) AND tenant_id = ${tenantId} LIMIT 10`;
        return res.status(200).json({ success: true, data: [...mats, ...mods] });
      }
      const result =
        await sql`SELECT m.*, c.nome as categoria_nome FROM materiais m LEFT JOIN erp_categories c ON m.categoria_id = c.id WHERE m.ativo = true AND m.tenant_id = ${tenantId} AND (c.tenant_id IS NULL OR c.tenant_id = ${tenantId}) ORDER BY m.nome ASC`;
      return res.status(200).json({ success: true, data: result });
    }
