...
const handleKanbanProducaoCore: TenantHandler = async (req, res) => {
  try {
    ...
    if (method === 'GET' && url.includes('/board')) {
      ...
      let queryStr = `
        SELECT 
          ...
          op.numero_op,
          op.orcamento_id as quotation_id,
        ...
        LEFT JOIN quotations o ON op.orcamento_id::text = o.id::text AND o.tenant_id = ep.tenant_id
    ...
    if (method === 'POST' && url.includes('/move-card')) {
      ...
      if (
        ...
        try {
          ...
          logger.error('[AUTO_CREATE_CUSTOS_REAIS_OP_ERROR]', err);
        }
      } else {
...
export const handleKanbanProducao = withTenant(handleKanbanProducaoCore);
