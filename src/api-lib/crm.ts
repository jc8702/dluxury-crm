...
export async function handleClients(req: any, res: any) {
  try {
    ...
    if (req.method === 'GET') {
      const result =
        await sql`SELECT id, nome, cpf, telefone, email, endereco, bairro, cidade, uf, tipo_imovel, comodos_interesse, origem, observacoes, status, created_at, razao_social, cnpj, municipio, situacao_cadastral FROM clients WHERE deleted_at IS NULL AND tenant_id = ${tenantId} ORDER BY created_at DESC`;
