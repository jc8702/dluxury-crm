...
export function calcularM2Peca(larguraCm: number, alturaCm: number): number {
...
export function precoCustoPorM2(precoChapa: number, larguraMm: number, alturaMm: number): number {
...
export function custoPeca(m2ComPerda: number, precoCustoM2: number): number {
...
export function calcularCustoTotal(params: {
  ...
  return params.custoPecas + params.custoFerragens + params.custoFita + params.custosExtras;
}

// Preço de venda com markup
// markup ex: 1.80 = custo × 1.80
export function calcularPrecoVenda(custoTotal: number, markup: number): number {
  return custoTotal * markup;
}

// Imposto sobre preço de venda (por dentro)
// aliquota ex: 0.06 para 6%
export function calcularImposto(precoVenda: number, aliquota: number): number {
...
export function markupParaMargem(margemDesejada: number, aliquota: number): number {
