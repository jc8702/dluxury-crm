...
export async function gerarPecas(p: Projeto): Promise<Peca[]> {
  ...
  if (p.gavetas > 0) {
    ...
    for (let i = 1; i <= p.gavetas; i++) {
      ...
      pecas.push({
        nome: `Gaveta ${i} Lateral E`,
        largura: p.profundidade - 50,
        altura: 120,
        espessura: 15,
        material: 'MDF',
      });
      pecas.push({
        nome: `Gaveta ${i} Lateral D`,
        largura: p.profundidade - 50,
        altura: 120,
        espessura: 15,
        material: 'MDF',
...
export function otimizarCorte(pecas: Peca[]): PlanoCorte[] {
  ...
  let pecasParaOtimizar = pecas
    .filter((p) => p.material === 'MDF')
    .map((p) => ({
      id: p.nome,
      nome: p.nome,
      largura: p.largura,
