  ...
  private static processText(text: string, resolve: any, reject: any, _file: File) {
    ...
    Papa.parse(text, {
      ...
      complete: (results) => {
        ...
        /* console.log('[CSVParser] Colunas detectadas:', fields) */ /* console.log('[CSVParser] Mapeamento:', mapping) */ // Fallback inteligente para o Nome (Função no SketchUp)
        if (!mapping.nome && fields.length > 0) {
          mapping.nome =
            fields.find((f) => {
              const sample = String(results.data[0]?.[f] || '');
              const isNumeric = !isNaN(
                parseFloat(sample.replace(/[^\d.,]/g, '').replace(',', '.')),
              );
          ...
          mapping.espessura =
            fields.find(
              (f) =>
                f !== mapping.largura &&
                f !== mapping.altura &&
                f !== mapping.quantidade &&
                !isNaN(parseFloat(String(results.data[0]?.[f]).replace(/[^\d.,]/g, ''))),
            ) || null;
        }

        const components = results.data
          .map((row: any, index) => this.normalizeRow(row, mapping, index + 1))
