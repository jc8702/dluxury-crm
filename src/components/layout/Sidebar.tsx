...
const Sidebar: React.FC<SidebarProps> = ({ isMobileOpen, onCloseMobile }) => {
  ...
  const renderMenuItem = (item: any, isSubItem = false) => {
            ...
            <div className="pl-4 pr-1 flex flex-col gap-1 border-l border-sidebar-border ml-5 my-1">
              {visibleSubItems.map((subItem: any) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`shrink-0 ${isSubActive ? 'text-primary-foreground' : 'text-sidebar-foreground'}`}
                    >
                      {subItem.icon}
                    </span>
    ...
    return (
      ...
      >
        <span
          className={`shrink-0 ${isActive ? 'text-primary-foreground' : 'text-sidebar-foreground'}`}
        >
          {item.icon}
        </span>
