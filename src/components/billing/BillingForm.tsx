...
const BillingModule: React.FC = () => {
  ...
  const currentData = filteredBillings.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage,
  );

  const totalEntradas = billings
    .filter((b) => b.tipo === 'entrada' && b.status === 'PAGO')
    .reduce((acc, b) => acc + b.valor, 0);
  const totalSaidas = billings
    .filter((b) => b.tipo === 'saida' && b.status === 'PAGO')
    .reduce((acc, b) => acc + b.valor, 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = {
        ...
        cliente:
          projects.find((p) => p.id === formData.projectId)?.clientName?.toUpperCase() || '-',
    ...
    return (
      <>
        ...
        <td style={{ padding: '0.75rem' }}>
          ...
          >
            {b.tipo === 'entrada' ? 'â†‘ Entrada' : 'â†“ Saída'}
          </span>
        </td>
        <td style={{ padding: '0.75rem', fontSize: '0.85rem' }}>{b.descricao || '-'}</td>
        <td
          style={{ padding: '0.75rem', fontSize: '0.85rem', color: 'hsl(var(--muted-foreground))' }}
        >
        ...
        <td style={{ padding: '0.75rem' }}>
          <div style={{ display: 'flex', gap: '0.3rem' }}>
            {[
              ...
              <div
                key={s.id}
                onClick={() => updateBilling(b.id, { status: s.id as any })}
                title={s.id}
                style={{
                  cursor: 'pointer',
                  width: '20px',
              ...
              >
                <div
                  style={{
                    ...
                    height: '5px',
                    borderRadius: '50%',
                    background: b.status === s.id ? 'white' : s.color,
                  }}
                />
              </div>
            ))}
    ...
    >
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: 'hsl(var(--muted-foreground))' }}>
            Controle de entradas e saídas por projeto.
          </p>
        </div>
        <button
          className="btn"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
        ...
        <div className="card glass" style={{ padding: '1.25rem', borderLeft: '3px solid #ef4444' }}>
          ...
          <h3 style={{ fontSize: '1.25rem', fontWeight: '800', color: '#ef4444' }}>
        ...
        <div
          ...
          style={{
            padding: '1.25rem',
            borderLeft: `3px solid ${saldo >= 0 ? '#d4af37' : '#ef4444'}`,
          }}
        >
      ...
      <div className="card">
        <div
          style={{ display: 'flex', gap: '1rem', marginBottom: '1.25rem', alignItems: 'center' }}
        >
          <input
            type="text"
            className="input"
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            {[
              ...
              { id: 'saida', label: 'â†“ Saídas', color: '#ef4444' },
            ].map((f) => (
              <button
                key={f.id}
                onClick={() => {
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  border:
                    typeFilter === f.id ? `1px solid ${f.color}` : '1px solid hsl(var(--border))',
        ...
        {totalPages > 1 && (
          ...
          >
            ...
            <span style={{ alignSelf: 'center', fontSize: '0.8rem' }}>
        ...
        >
              ...
              <button
                ...
                onClick={() =>
                  setFormData({
                    ...formData,
                    tipo: t,
                    categoria: t === 'entrada' ? 'sinal' : 'material',
                  })
                }
                style={{
                  flex: 1,
                  ...
                  background:
                    formData.tipo === t ? (t === 'entrada' ? '#10b981' : '#ef4444') : 'transparent',
                  color: formData.tipo === t ? 'white' : 'hsl(var(--muted-foreground))',
                }}
              >
                {t === 'entrada' ? 'â†‘ Entrada' : 'â†“ Saída'}
              </button>
            ))}
          </div>
          ...
          <div>
            ...
            <input
              style={inputStyle}
              required
              placeholder="Ex: Sinal Cozinha Maria"
