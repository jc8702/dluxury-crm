...
const Inventory: React.FC = () => {
  ...
  const { materiais, categorias, movimentacoes, removeMaterial, reloadInventoryData } =
    useInventory();
              ...
              {filteredMaterials.map((m) => {
                ...
                return (
                  <MaterialCard
                    ...
                    material={m}
                    categoria={
                      categorias ? categorias.find((c) => c.id === m.categoria_id) : undefined
                    }
                  ...
                  <tbody className="divide-y divide-border">
                    {filteredMaterials.map((m) => {
                      ...
                      const cat = categorias
                        ? categorias.find((c) => c.id === m.categoria_id)
                        : undefined;
                      const equivalencia =
                        Number(m.estoque_atual || 0) * Number(m.fator_conversao || 1);

                      return (
                        <tr key={m.id} className="hover:bg-muted/10 transition-colors">
                          <td className="p-4 font-mono text-xs text-foreground">{m.sku || '-'}</td>
                          <td className="p-4 font-semibold text-foreground">
                            {m.nome || 'Material sem nome'}
                          </td>
                          <td className="p-4 text-muted-foreground">{cat?.nome || '-'}</td>
                          <td className="p-4 font-bold text-foreground">
                            {m.estoque_atual || 0} {m.unidade_compra}
                          </td>
                ...
                {movimentacoes.length > 0 ? (
                  movimentacoes.map((mov) => (
                    <tr key={mov.id} className="hover:bg-muted/10 transition-colors">
                      ...
                      <td className="p-4 font-bold text-foreground">
                        {mov.tipo === 'saida' ? '-' : '+'}
                        {mov.quantidade} {mov.material_unidade}
                      </td>
                      ...
                      <td className="p-4 text-muted-foreground">{mov.criado_por}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
