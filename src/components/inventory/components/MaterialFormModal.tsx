...
const MaterialFormModal: React.FC<MaterialFormModalProps> = ({ material, onClose, onSuccess }) => {
  const { categorias, materiais, addMaterial, updateMaterial } = useInventory();
  ...
  const { fornecedores } = useInventory();

  useEffect(() => {
    if (!material && form.categoria_id) {
      const prefix = form.categoria_id;
      const matCat = materiais.filter(
        (m) => m.categoria_id === form.categoria_id && m.sku && m.sku.startsWith(`${prefix}-`),
      );
      let nextNum = 1;
      if (matCat.length > 0) {
  ...
  const handleDimensionChange = (field: 'largura_mm' | 'altura_mm', val: number) => {
    setForm((prev) => {
      ...
      const fatorPerda = next.unidade_compra === 'chapa' ? 1.1 : 1.05;
  ...
  const handlePriceChange = (field: 'preco_custo' | 'margem_lucro', value: number) => {
    setForm((prev) => {
      ...
      const fatorPerda = prev.unidade_compra === 'chapa' ? 1.1 : 1.05;
      const calcVenda = (custoNF / fator) * fatorPerda + margemVal;
            ...
            <div>
              ...
              <span className="text-sm font-semibold">
                ...
                <span className="text-primary font-bold">
                  R$ {calculos.custoEfetivo.toFixed(2)}
                </span>{' '}
