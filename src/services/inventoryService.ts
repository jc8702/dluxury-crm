...
export const inventoryService = {
  async getItems(filtro = 'todos', busca = ''): Promise<{ items: EstoqueGranularItem[] }> {
    try {
      ...
      return await apiCall<{ items: EstoqueGranularItem[] }>(`estoque/items?${params.toString()}`);
    } catch (error) {
      throw new AppError('Erro ao buscar itens do estoque', 400, error);
    }
  },
  ...
  async registrarMovimento(data: {
    ...
    return apiCall<{ success: boolean; saldo_novo: number; quantidade_total: number }>(
      'estoque/registrar-movimento',
      'POST',
      data,
    );
  },

  async matchSKUsEmLote(
    quotation_id: string,
    itens_csv: Array<{
      sku_promob: string;
      descricao: string;
    ...
    try {
      ...
      throw new AppError('Erro ao fazer match de SKUs em lote', 400, error);
    }
  },
};
