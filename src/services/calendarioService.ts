...
export type TipoEventoType = 'orcamento' | 'prazo_entrega' | 'lembrete_compra' | 'tarefa' | 'reuniao';
...
export const calendarioService = {
  ...
  async getEventos(mes: number, ano: number, filtroTipo?: string): Promise<EventoCalendarioType[]> {
    ...
    const data = await apiCall<{ eventos: EventoCalendarioType[] }>(`calendario/eventos?${params.toString()}`);
    return data.eventos || [];
  },
   ...
   */
  async updateEvento(id: number, data: { concluido?: boolean; cor_categoria?: string }): Promise<any> {
  ...
  async verificarLembretes(): Promise<{ sucesso: boolean; notificacoes_disparadas: number }> {
    return apiCall<{ sucesso: boolean; notificacoes_disparadas: number }>('calendario/verificar-lembretes');
  }
};
