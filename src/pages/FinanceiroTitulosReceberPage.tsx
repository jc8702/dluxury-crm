...
export default function FinanceiroTitulosReceberPage() {
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto animate-fade-in">
      ...
      <div className="glass-elevated overflow-hidden mb-10">
        ...
        <div className="px-6 py-4 border-t border-white/5 bg-white/[0.02] flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-black text-muted-foreground uppercase tracking-widest">
          ...
          <div className="flex gap-2">
             <Button variant="outline" size="sm" className="h-9 px-4 uppercase font-black italic" disabled={page === 1} onClick={() => setPage(page-1)}>Anterior</Button>
             <Button variant="outline" size="sm" className="h-9 px-4 uppercase font-black italic" disabled={page * perPage >= total} onClick={() => setPage(page+1)}>Próxima</Button>
          </div>
        </div>
      </div>

      {/* Modal Antecipação */}
      <Modal isOpen={!!antecipacaoModal} onClose={() => setAntecipacaoModal(null)} title="Simulador de Antecipação Industrial" size="md">
        {antecipacaoModal && (() => {
          const hoje = new Date();
          ...
          return (
            <div className="space-y-6">
              ...
              <div className="flex gap-4 justify-end">
                <Button variant="outline" onClick={() => setAntecipacaoModal(null)}>FECHAR</Button>
      ...
      {/* Modal Baixa */}
      <Modal isOpen={!!baixaModal} onClose={() => setBaixaModal(null)} title="Registrar Recebimento Industrial" size="md">
        {baixaModal && (() => {
          const hoje = new Date();
          ...
          return (
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block ml-1">Conta Bancária de Destino</label>
                <select id="conta-interna-id-receber" className="w-full bg-background border border-border rounded-xl px-4 py-3 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary uppercase font-bold">
      ...
      <Modal isOpen={!!editModal} onClose={() => setEditModal(null)} title="Manutenção de Título Industrial" size="lg">
        <div className="space-y-6">
