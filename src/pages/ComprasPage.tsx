      ...
      <header className="flex justify-between items-center pb-4 border-b border-border">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Fluxos de suprimentos, histórico de preços e reposição de estoque.
          </p>
        </div>
        <Button
          variant="primary"
...
const SugestoesGrid: React.FC<{ sugestoes: any[]; onAction: () => void }> = ({
  sugestoes,
  onAction: _onAction,
}) => (
  <Card>
            ...
            sugestoes.map((s) => (
              <tr key={s.material_id} className="hover:bg-muted/10 transition-colors">
                ...
                <td className="p-4 text-center">
                  <span
                    className={`font-bold ${s.estoque_atual <= 0 ? 'text-destructive' : 'text-warning'}`}
                  >
                    {s.estoque_atual} {s.unidade}
                  </span>
                </td>
                <td className="p-4 text-center text-muted-foreground">
                  {s.estoque_minimo} {s.unidade}
                </td>
...
}> = ({ pedidos, onView, getStatusColor, getStatusLabel }) => (
  <Card>
    <CardContent className="p-0 overflow-x-auto custom-scrollbar">
      <table className="w-full border-collapse text-sm">
        <thead>
...
const PedidoModal: React.FC<{ pedido: any; onClose: () => void; onSave: () => void }> = ({
  pedido,
  onClose,
  onSave,
}) => {
  ...
  const addItem = () => {
    ...
    const newTotal =
      newItens.reduce((acc, i) => acc + i.subtotal, 0) + (Number(formData.frete) || 0);
  ...
  const removeItem = (index: number) => {
    ...
    const newTotal =
      newItens.reduce((acc: number, i: any) => acc + i.subtotal, 0) + (Number(formData.frete) || 0);
    setFormData({ ...formData, itens: newItens, valor_total: newTotal });
  };

  const handleUpdateItemField = (
    index: number,
    field: 'quantidade_pedida' | 'preco_unitario',
    value: number,
  ) => {
              ...
              <table className="w-full border-collapse text-sm">
                    ...
                    formData.itens?.map((itm: any, idx: number) => (
                      <tr key={idx} className="hover:bg-muted/10 transition-colors">
                        ...
                        <td className="p-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleUpdateItemField(idx, 'preco_unitario', Number(e.target.value))
                              }
                        ...
                        <td className="p-3 text-right font-bold text-foreground">
                          R$ {Number(itm.subtotal).toFixed(2)}
                        </td>
                ...
                {formData.itens?.length > 0 && (
                  <tfoot className="bg-muted/20 border-t border-border font-semibold">
                    <tr>
                      <td colSpan={3} className="text-right p-4 text-muted-foreground">
                        TOTAL DO PEDIDO:
                      </td>
                      <td className="text-right p-4 font-bold text-primary text-base">
                        R${' '}
                        {Number(formData.valor_total).toLocaleString('pt-BR', {
