...
const CuttingPlanPage: React.FC = () => {
  ...
  const [plano, setPlano] = useState<PlanoCorteData>({
    nome: 'Novo Plano de Corte',
    status: 'rascunho',
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const sobrasAproveitaveis = resultado.sobrasGeradas.filter(
        (s) => s.aproveitavel && s.largura * s.altura >= areaMinima,
      );

      if (sobrasAproveitaveis.length > 0) {
        await Promise.all(
          sobrasAproveitaveis.map(async (s) => {
            const grupo = resultado.grupos.find((g) =>
              g.superficies.some((sup) => sup.id === s.superficieId),
            );
  ...
  const handleAprovarProducao = async () => {
    ...
    const isConfirmed = await confirmAction({
      title: 'APROVAR PRODUÃ‡ÃƒO',
      description:
        'CONFIRMA A APROVAÃ‡ÃƒO? ESTA AÃ‡ÃƒO REALIZA A BAIXA DE ESTOQUE E GERA A ORDEM DE PRODUÃ‡ÃƒO.',
    });
    ...
    try {
      ...
      if (retalhosUtilizadosIds.length > 0) {
        await Promise.all(
          retalhosUtilizadosIds.map((id) => retalhosRepository.usarRetalho(id, plano.id!)),
        );
      }

      const totalPecas = resultado.grupos.reduce(
        (acc, g) => acc + g.superficies.reduce((accS, s) => accS + s.pecasPositionadas.length, 0),
        0,
      );
      ...
      for (const orc of approvedOrcs) {
        ...
        pecasParaProcessar.forEach((peca) => {
          ...
          novasPecas.push({
            ...
            podeRotacionar:
              peca.sentido_veio !== 'longitudinal' && peca.sentido_veio !== 'transversal',
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0A0A0A] text-white overflow-hidden animate-fade-in">
      ...
      <header className="flex items-center justify-between p-6 bg-black/40 border-b border-white/5 backdrop-blur-md z-30">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="btn btn-outline h-12 px-5 group"
          >
          ...
          <button
            onClick={() => setShowIndustrialImport(true)}
            className="btn btn-outline h-12 px-5 border-blue-500/20 text-blue-400 hover:bg-blue-500/10"
          >
            <FileUp size={18} className="mr-2" /> INDUSTRIAL
          </button>
          <button
            onClick={() => setShowImportModal(true)}
            className="btn btn-outline h-12 px-5 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10"
          >
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="space-y-1">
                      ...
                      <h4 className="text-base font-black italic tracking-tight">
                        {g.nomeMaterial}
                      </h4>
                    </div>
                    <button
                  ...
                  <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/5">
                    <div>
                      <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest italic block mb-1">
                        ALTURA
                      </span>
                      <input
                        type="number"
                        ...
                        onChange={(e) =>
                          updateGrupo(g.id, { alturaChapaMm: Number(e.target.value) })
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateGrupo(g.id, { larguraChapaMm: Number(e.target.value) })
                        }
                        className="w-full bg-transparent border-none p-0 text-sm font-black text-white focus:ring-0"
                      />
                    </div>
                    <div>
                      <span className="text-[9px] font-black text-primary uppercase tracking-widest italic block mb-1">
                        ESP.
                      </span>
                      <input
                        type="number"
            ...
            <div className="h-[400px] border-t border-white/5 bg-black/40 p-6 flex flex-col">
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.3em] italic">
                    02. PEÃ‡AS ({pecas.filter((p) => p.grupoMaterialId === activeGrupo.id).length})
                  </h3>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-2">
                {pecas
                  .filter((p) => p.grupoMaterialId === activeGrupo.id)
                  .map((p) => (
                    <div
                      key={p.id}
                      className="bg-white/5 hover:bg-white/10 border border-white/5 p-3 rounded-2xl flex items-center gap-4 transition-all group"
                    >
                      <input
                        value={p.descricao}
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          type="number"
                          value={p.larguraMm}
                          onChange={(e) => updatePeca(p.id, { larguraMm: Number(e.target.value) })}
                          className="w-11 bg-black/40 border-none rounded-lg text-center text-[10px] font-bold p-1 focus:ring-1 focus:ring-primary/40"
                        />
                        <span className="text-[8px] text-white/20">Ã—</span>
                        <input
                          type="number"
                          value={p.alturaMm}
                          onChange={(e) => updatePeca(p.id, { alturaMm: Number(e.target.value) })}
                          className="w-11 bg-black/40 border-none rounded-lg text-center text-[10px] font-bold p-1 focus:ring-1 focus:ring-primary/40"
                        />
                      </div>
                      <div className="w-10 flex flex-col items-center">
                        <span className="text-[7px] font-black text-primary opacity-50 uppercase tracking-tighter">
                          QTD
                        </span>
                        <input
                          type="number"
            ...
            {activeResultadoGrupo?.superficies.map((s, idx) => (
              ...
              >
                <div className="flex flex-col items-start leading-none">
                  <span className="mb-1">
                    {s.tipo === 'retalho' ? 'RETALHO' : `CHAPA ${idx + 1}`}
                  </span>
                  <span
                    className={`text-[8px] ${activeChapaIdx === idx ? 'text-black/60' : 'text-primary/60'}`}
                  >
                    {Number(s.aproveitamentoPct || 0).toFixed(1)}% EFICIÃŠNCIA
                  </span>
                </div>
              </button>
            ))}
            ...
            {activeSuperficie ? (
              ...
              <div className="text-center space-y-6 opacity-20 group">
                ...
                <div className="space-y-2">
                  <h2 className="text-2xl font-black italic tracking-[0.3em] uppercase">
          ...
          {resultado && (
            <div className="p-8 pb-10">
              <div className="glass-elevated bg-primary/5 rounded-[2.5rem] border border-primary/20 p-8 flex items-center justify-between backdrop-blur-xl shadow-2xl shadow-black/40 animate-slide-up">
                <div className="flex gap-12">
                  <div className="space-y-1">
                    <div className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] italic flex items-center gap-2">
                      <TrendingUp className="w-3 h-3 text-primary" /> EFICIÃŠNCIA GLOBAL
                    </div>
                  ...
                  <div className="space-y-1">
                    <div className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] italic flex items-center gap-2">
                      <CheckCircle className="w-3 h-3 text-emerald-400" /> PEÃ‡AS PROCESSADAS
                    </div>
                    <div className="text-4xl font-black italic tracking-tighter">
                ...
                >
                  <Download size={24} /> EXPORTAR PRODUÃ‡ÃƒO
                </button>
              </div>
            </div>
          )}
      ...
      {/* MODAL: HISTÃ“RICO DE PLANOS */}
      <Modal
        isOpen={showPlanosModal}
        onClose={() => setShowPlanosModal(false)}
        title="HISTÃ“RICO DE ENGENHARIA"
        width="700px"
      >
              ...
              >
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <span className="text-[8px] font-black px-2 py-0.5 bg-white/10 rounded text-muted-foreground uppercase tracking-widest">
                      ID: {p.id?.slice(-6)}
                    </span>
                    <h4 className="text-lg font-black italic tracking-tight group-hover:text-primary transition-colors">
                      {p.nome}
                    </h4>
                  </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 p-4">
          ...
          <div className="space-y-6">
            ...
            <div className="glass-elevated p-8 rounded-[2.5rem] border border-white/5 bg-blue-500/[0.02]">
              ...
              <p className="text-[10px] text-muted-foreground mt-6 text-center leading-relaxed uppercase tracking-widest font-bold">
                Suporte nativo para arquivos .DAE, .XML e .CSV exportados do SketchUp, Promob e
                TopSolid.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6 flex flex-col h-full">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-sm font-black italic uppercase tracking-widest text-emerald-400">
                02. Projetos do Sistema (TAG)
              </h3>
            </div>
            ...
            <div className="flex-1 glass-elevated p-6 rounded-[2.5rem] border border-white/5 overflow-hidden flex flex-col bg-emerald-500/[0.02]">
              <div className="mb-6 relative">
                ...
                <input
                  className="w-full bg-black/40 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-xs font-bold"
                  placeholder="Pesquisar por TAG ou Cliente..."
                />
              </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs font-black text-muted-foreground uppercase tracking-widest mt-1">
                        ðŸ‘¤ {p.client_name || p.cliente_nome || 'Cliente Interno'}
                      </p>
                    </div>
                    ...
                    <div className="flex justify-between items-end border-t border-white/5 pt-4">
                      ...
                      <div className="text-base font-black italic text-emerald-400">
        ...
        <div className="p-4 pt-0">
          <button
            className="btn btn-outline w-full h-14 font-black italic tracking-widest mt-6"
