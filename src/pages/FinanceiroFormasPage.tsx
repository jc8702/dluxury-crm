...
const FinanceiroFormasPage: React.FC = () => {
  ...
  const getTipoBadgeVariant = (tipo: string) => {
    switch (tipo?.toLowerCase()) {
      ...
      case 'debito': return 'outline';
      case 'boleto': return 'secondary';
  ...
  return (
    <div className="page-container anim-fade-in" style={{ padding: '1rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <Button variant="outline" onClick={fetch} disabled={loading} style={{ fontSize: '0.85rem' }} aria-label="Atualizar lista">
            <RefreshCw size={16} className={loading ? 'anim-spin' : ''} style={{ marginRight: '0.25rem' }} /> ATUALIZAR
          </Button>
          <Button variant="primary" onClick={openNew} style={{ fontSize: '0.85rem' }} aria-label="Nova forma de pagamento">
      ...
      <Card style={{ overflow: 'hidden' }}>
        <CardHeader>
          <CardTitle style={{ fontSize: '1rem', fontWeight: 800 }}>Métodos de Pagamento Ativos</CardTitle>
        </CardHeader>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {formas.map(f => (
                    <tr 
                      key={f.id} 
                      style={{ borderBottom: '1px solid hsl(var(--border))', transition: 'background 0.2s' }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div>
            ...
            >
              ...
              <option value="credito" style={{ background: '#1e293b' }}>Cartão de Crédito</option>
              <option value="debito" style={{ background: '#1e293b' }}>Cartão de Débito</option>
              <option value="boleto" style={{ background: '#1e293b' }}>Boleto</option>
              <option value="transferência" style={{ background: '#1e293b' }}>Transferência Bancária / TED</option>
            </select>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <div>
              ...
              <Input 
                ...
                onChange={e => setForm({...form, prazo_compensacao_dias: Math.max(0, Number(e.target.value))})} 
              />
            </div>
          </div>
