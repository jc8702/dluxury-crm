    ...
    <div className="page-container anim-fade-in" style={{ maxWidth: '1400px', margin: '0 auto', padding: '1rem' }}>
      ...
      <header style={{ marginBottom: '2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'hsl(var(--muted-foreground))', fontSize: '0.95rem', margin: 0 }}>Projeção estratégica de liquidez e saúde financeira</p>
        </div>

        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
           {/* Controles combinados */}
           <div className="card glass" style={{ padding: '0.35rem', display: 'flex', gap: '0.35rem', borderRadius: '12px', background: 'hsl(var(--surface-elevated))', border: '1px solid hsl(var(--border))' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.25rem', marginBottom: '2.5rem' }}>
        {[
          ...
          { label: 'Total Saídas', value: periodos.reduce((s, p) => s + p.despesas, 0), color: 'hsl(var(--destructive))', icon: ArrowRight },
          { label: 'Ponto de Equilíbrio (Final)', value: periodos[periodos.length - 1]?.saldo_projetado || saldoAtual, color: 'hsl(var(--accent))', icon: TrendingUp },
        ].map((k, i) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem', alignItems: 'start' }} className="md:grid-cols-[1fr_380px]">
        <style>{`
          ...
          {viewMode === 'chart' ? (
            <Card>
              <CardHeader style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <CardTitle style={{ fontSize: '1rem', fontWeight: 800 }}>PROJEÇÃO DE DISPONIBILIDADE</CardTitle>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                   <Button onClick={() => setViewMode('chart')} variant={viewMode === 'chart' ? 'primary' : 'outline'} size="sm"><Grid size={16} /></Button>
                   <Button onClick={() => setViewMode('table')} variant={viewMode === 'table' ? 'primary' : 'outline'} size="sm"><List size={16} /></Button>
                </div>
              </CardHeader>
            ...
            <Card style={{ overflow: 'hidden' }}>
              <CardHeader style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <CardTitle style={{ fontSize: '0.9rem', fontWeight: 800 }}>DETALHAMENTO TEMPORAL</CardTitle>
                <div style={{ fontSize: '0.75rem', color: 'hsl(var(--muted-foreground))' }}>* Clique na linha para selecionar o período</div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                   <Button onClick={() => setViewMode('chart')} variant={viewMode === 'chart' ? 'primary' : 'outline'} size="sm"><Grid size={16} /></Button>
                   <Button onClick={() => setViewMode('table')} variant={viewMode === 'table' ? 'primary' : 'outline'} size="sm"><List size={16} /></Button>
                </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: 'hsl(var(--surface))' }}>
                      <th style={{ padding: '1rem', textAlign: 'left', fontSize: '0.7rem', fontWeight: 800, color: 'hsl(var(--muted-foreground))', textTransform: 'uppercase' }}>Período</th>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
           <Card>
              <CardHeader style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '0.5rem' }}>
                 <Calendar color="hsl(var(--primary))" size={20} />
                 <CardTitle style={{ fontSize: '1rem', fontWeight: 800, margin: 0 }}>DETALHES DO PERÍODO</CardTitle>
              </CardHeader>
                ...
                {loading ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                     <div>
                        ...
                        <div style={{ fontSize: '0.7rem', color: 'hsl(var(--muted-foreground))', marginBottom: '1rem' }}>{new Date(selectedPeriod.inicio).toLocaleDateString()} até {new Date(selectedPeriod.fim).toLocaleDateString()}</div>
                     </div>
                     ...
                     <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                          {selectedPeriod.titulos_receber.length === 0 ? (
                            <div style={{ padding: '1rem', textAlign: 'center', fontSize: '0.75rem', color: 'hsl(var(--muted-foreground))', background: 'hsl(var(--surface))', borderRadius: '8px' }}>Nenhuma entrada.</div>
                     ...
                     <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                          {selectedPeriod.titulos_pagar.length === 0 ? (
                            <div style={{ padding: '1rem', textAlign: 'center', fontSize: '0.75rem', color: 'hsl(var(--muted-foreground))', background: 'hsl(var(--surface))', borderRadius: '8px' }}>Nenhuma saída.</div>
           ...
           <Card style={{ background: 'hsl(var(--primary) / 0.02)', border: '1px dashed hsl(var(--primary) / 0.19)' }}>
              <CardContent style={{ padding: '1.25rem' }}>
                ...
                <p style={{ fontSize: '0.65rem', color: 'hsl(var(--muted-foreground))', marginTop: '0.5rem', margin: 0 }}>Este valor representa o saldo final projetado após todas as movimentações previstas até o fim deste período.</p>
              </CardContent>
           </Card>
        </aside>
