      ...
      <div className="max-w-[1600px] mx-auto animate-in fade-in duration-700">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/financeiro/fluxo-caixa"
              className="px-5 py-2.5 rounded-lg text-xs font-bold bg-amber-500 text-black hover:bg-amber-400 transition-all shadow-[0_0_25px_rgba(245,158,11,0.2)] flex items-center gap-2"
            >
        ...
        <nav className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3 mb-10">
          {atalhos.map((item, i) => (
            ...
            >
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider text-foreground group-hover:text-primary-foreground">
                {item.title}
              </span>
            </Link>
          ))}
            ...
            <div className="grid grid-cols-12 gap-6 pb-20">
              ...
              <div className="col-span-12 md:col-span-6 glass p-6 rounded-2xl border-border relative overflow-hidden flex flex-col justify-between group">
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div
                    className={`p-2 rounded-lg ${(stats.capital_de_giro || 0) >= 0 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}
                  >
                    <Activity size={20} />
                  </div>
                </div>
                ...
                <div className="flex items-baseline gap-4">
                  <div
                    className={`text-4xl font-black tracking-tighter ${(stats.capital_de_giro || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                  >
                ...
                <div className="mt-4 flex items-center gap-4 text-[10px] font-bold text-muted-foreground border-t border-border pt-4">
                  ...
                  <div className="flex items-center gap-1">
                    <div className="w-2 h-2 rounded-full bg-rose-500/50" /> {fmt(stats.a_pagar_30d)}{' '}
                    Exigíveis
                  </div>
                </div>
              </div>
                      ...
                      {(stats.proximos_vencimentos || []).length === 0 ? (
                        ...
                        stats.proximos_vencimentos.map((v, i) => (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-1.5 h-1.5 rounded-full ${v.tipo === 'pagar' ? 'bg-rose-500' : 'bg-emerald-500'}`}
                              />
                              <div>
                            ...
                            <div
                              className={`text-[12px] font-black font-mono ${v.tipo === 'pagar' ? 'text-rose-400' : 'text-emerald-400'}`}
                            >
                  ...
                  <div className="glass p-6 rounded-2xl border-border">
                    ...
                    <div className="space-y-3">
                      {(stats.top5_inadimplentes || []).map((cli, i) => (
                        <div
                          key={i}
                          className="flex justify-between items-center p-3 rounded-xl bg-rose-500/[0.03] border border-rose-500/10 hover:bg-rose-500/5 transition-colors"
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-6 h-6 rounded-lg bg-rose-500/10 flex items-center justify-center text-[10px] font-black text-rose-500">
                              {i + 1}
                            </div>
                            <div>
                              <div className="text-[11px] font-bold text-foreground">
                              ...
                              <div className="text-[10px] text-rose-400/60 font-medium uppercase tracking-tighter">
              ...
              <div className="col-span-12 md:col-span-4 space-y-6">
                ...
                <div className="glass p-6 rounded-2xl border-border bg-gradient-to-br from-muted/20 to-transparent">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <Link
                      to="/financeiro/contas"
                      className="text-amber-500 p-1 hover:bg-amber-500/10 rounded transition-colors"
                    >
                    ...
                    {(stats.contas || []).map((c, i) => (
                      <div key={i} className="flex flex-col gap-1">
                        <div className="flex justify-between items-center text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                          <span>{c.nome}</span>
                          <span
                            className={c.saldo_atual >= 0 ? 'text-emerald-500' : 'text-rose-500'}
                          >
                            {fmt(c.saldo_atual)}
                          </span>
                        </div>
                  ...
                  <div className="mt-8 p-4 rounded-xl bg-amber-500/5 border border-amber-500/10 flex justify-between items-center">
                    <div>
                      <div className="text-[9px] font-black text-amber-500 uppercase tracking-[0.2em] mb-1">
                        Saldo Consolidado
                      </div>
                      <div className="text-xl font-black tracking-tighter text-amber-500 font-mono">
