...
export default function NotificacoesPage() {
  ...
  const [priorityFilter, setPriorityFilter] = useState<'todas' | 'critica' | 'alta' | 'normal'>('todas');
  ...
  const getPriorityStyle = (priority: string) => {
    switch (priority) {
      case 'critica': return { text: 'text-red-400', border: 'border-red-500/30', bg: 'bg-red-500/10' };
      case 'alta': return { text: 'text-amber-400', border: 'border-amber-500/30', bg: 'bg-amber-500/10' };
      case 'normal': return { text: 'text-blue-400', border: 'border-blue-500/30', bg: 'bg-blue-500/10' };
      default: return { text: 'text-muted-foreground', border: 'border-border', bg: 'bg-muted/10' };
    }
  };
  ...
  const getIcon = (type: string, priority: string) => {
    ...
    switch (type) {
      case 'estoque_critico': return <Package className={`w-5 h-5 ${style.text}`} />;
      case 'prazo_projeto': return <Calendar className={`w-5 h-5 ${style.text}`} />;
      case 'orcamento_sem_resposta': return <FileText className={`w-5 h-5 ${style.text}`} />;
      case 'garantia_pendente': return <Mail className={`w-5 h-5 ${style.text}`} />;
      case 'cobranca_vencida': return <ShieldAlert className={`w-5 h-5 ${style.text}`} />;
      default: return <Bell className={`w-5 h-5 ${style.text}`} />;
    }
  };
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto animate-fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6 border-b border-border pb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground italic">Monitoramento e Alertas</span>
          </div>
          <h1 className="text-5xl font-black italic tracking-tighter">
            CENTRAL DE <span className="text-primary underline decoration-primary/30 underline-offset-8">ALERTAS</span>
          </h1>
          <p className="text-muted-foreground mt-4 font-medium max-w-xl leading-relaxed">
            Monitoramento em tempo real de gargalos operacionais, estoques mínimos, prazos de projetos e cobranças financeiras.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button 
            ...
            isLoading={checking}
          >
            <RefreshCw className={`w-4 h-4 mr-2 transition-transform group-hover:rotate-180 ${checking ? 'animate-spin' : ''}`} /> VERIFICAR NOVOS
          </Button>
          <Button 
            variant="primary" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
        {[
          { label: 'Alertas Pendentes', value: stats.naoLidas, color: 'text-primary', border: 'border-primary/20', desc: 'Aguardando verificação' },
          { label: 'Risco Crítico', value: stats.criticas, color: 'text-red-400', border: 'border-red-500/30', desc: 'Cobranças e estoques zerados' },
          { label: 'Urgências (Alta)', value: stats.altas, color: 'text-amber-400', border: 'border-amber-500/30', desc: 'Prazos e garantias' },
          { label: 'Total Histórico', value: stats.total, color: 'text-blue-400', border: 'border-blue-500/30', desc: 'Notificações no cache atual' }
        ].map((card, i) => (
          <div key={i} className={`glass-elevated p-6 rounded-2xl border-l-4 ${card.border} relative overflow-hidden group`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-3 space-y-6">
          <Card className="glass-elevated rounded-3xl border border-border">
            <CardHeader>
              <CardTitle className="text-xs font-black tracking-widest uppercase italic">Filtro de Leitura</CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col gap-2 p-4 pt-0">
              ...
              >
                <div className={`w-2.5 h-2.5 rounded-full mr-3 ${filter === 'não_lidas' ? 'bg-black' : 'bg-primary'}`} />
              ...
              >
                <div className={`w-2.5 h-2.5 rounded-full mr-3 ${filter === 'todas' ? 'bg-black' : 'bg-muted-foreground'}`} />
          ...
          <Card className="glass-elevated rounded-3xl border border-border">
            <CardHeader>
              <CardTitle className="text-xs font-black tracking-widest uppercase italic">Filtrar por Gravidade</CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col gap-2 p-4 pt-0">
              {[
            ...
            <CardContent className="p-0 min-h-[500px]">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-[500px] gap-6 text-center">
                  ...
                  <div>
                    <h3 className="text-2xl font-black italic tracking-tight uppercase">TUDO MONITORADO</h3>
                    <p className="text-muted-foreground mt-2 max-w-sm font-medium text-xs uppercase tracking-wider leading-relaxed">
                      Nenhuma notificação identificada para os parâmetros selecionados. Sua operação está sob controle.
                    </p>
                  </div>
                </div>
              ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-4 mb-1">
                            <div className="flex flex-wrap items-center gap-3 min-w-0">
                              ...
                              <span className={`text-[8px] font-black px-2.5 py-0.5 rounded-md border tracking-widest uppercase ${
                                n.prioridade === 'critica' ? 'bg-red-500/10 border-red-500/20 text-red-400' :
                                n.prioridade === 'alta' ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' :
                            ...
                            <span className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-wider flex items-center gap-1.5 shrink-0">
                              <Clock size={12} className="text-primary opacity-60" /> {new Date(n.created_at!).toLocaleString('pt-BR')}
                            </span>
                          </div>
                          ...
                          <div className="flex gap-2 items-center">
                            {n.url_destino && (
                              ...
                              >
                                RESOLVER ALERTA <ArrowRight size={14} className="ml-2 group-hover:translate-x-1 transition-transform" />
                              </Button>
                            )}
                            {!n.lida && (
                              <Button 
