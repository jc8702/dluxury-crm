...
async function getEmbedding(text: string, apiKey: string): Promise<number[]> {
  ...
  if (!values || values.length !== 768) {
    throw new Error(
      `Tamanho de vetor retornado inválido. Esperado 768, recebido: ${values?.length}`,
    );
  }
  return values;
}
...
async function seed() {
  /* console.log('--- INICIANDO SEEDING DA BASE DE CONHECIMENTO VETORIAL RAG ---') */ if (!db) {
    console.error(
      '❌ Erro: Conexão com o banco de dados (db) não pôde ser estabelecida ou DATABASE_URL está vazia.',
    );
    process.exit(1);
  }
  ...
  if (!apiKey) {
    console.warn(
      '⚠️ Aviso: Chave de API do Gemini (GEMINI_API_KEY) ausente. Usando embeddings simulados.',
    );
    useSimulation = true;
  }

  try {
    /* console.log('Limpando dados antigos da base de conhecimento para evitar duplicados...') */ await db.delete(
      conhecimentoMarcenaria,
    );
    /* console.log('Tabela limpa com sucesso!') */ /* console.log(`Populando ${CONHECIMENTO_ITEMS.length} tópicos na base de marcenaria...`) */ let _inseridos = 0;
    for (let i = 0; i < CONHECIMENTO_ITEMS.length; i++) {
      ...
      if (useSimulation) {
        ...
        try {
          ...
          console.warn(
            `⚠️ Falha ao obter embedding real via API para "${item.titulo}": ${apiError.message}`,
          );
    ...
    /* console.log('🎉 Seeding do banco de RAG concluído com sucesso!') */ process.exit(0);
  } catch (error) {
