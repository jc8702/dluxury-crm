...
export const quotations = pgTable(
  'quotations',
  {
    ...
    taxaFinanceiraPercentual: decimal('taxa_financeira_percentual', {
      precision: 5,
      scale: 2,
    }).default('0'),
...
export const quotationBom = pgTable(
  'quotation_bom',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }),
  ...
  (table) => {
    return {
      ...
      tenantIdx: index('idx_quotation_bom_tenant_id').on(table.tenantId),
    };
  },
);
...
export const quotationItemsRelations = relations(quotationItems, ({ one, many }) => ({
  ...
  skuEngenharia: one(skuEngenharia, {
    fields: [quotationItems.skuEngenhariaId],
    references: [skuEngenharia.id],
  }),
  skuComponente: one(skuComponente, {
    fields: [quotationItems.skuComponenteId],
...
export const quotationBomRelations = relations(quotationBom, ({ one }) => ({
  ...
  componente: one(skuComponente, {
    fields: [quotationBom.skuComponenteId],
    references: [skuComponente.id],
  }),
}));
