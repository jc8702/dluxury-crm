...
export const objetivoVisitaEnum = [
  'apresentacao',
  'medicao',
  'instalacao',
  'pos_venda',
...
export const eventos = pgTable(
  ...
  (table) => ({
    ...
    idxStatusVisita: index('idx_eventos_status_visita')
      .on(table.statusVisita)
      .where(sql`tipo = 'visita'`),
...
export const eventosHistorico = pgTable(
  'eventos_historico',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }),
