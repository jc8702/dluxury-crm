...
export default async function handler(req: any, res: any) {
    ...
    try {
        // 1. Buscar dados completos incluindo a relação com o cliente e itens ordenados
        ...
        // 2. Gerar PDF com layout premium de proposta comercial
        ...
        doc.text(dataFormatada, pageWidth - 20, 33, { align: 'right' });
        ...
        const tableBody = (orcamento.itens || []).map((item: any) => [
            { 
                content: `${item.nomeCustomizado || item.skuEngenharia?.nome || 'Item Avulso'}\n${item.skuCodigo ? `SKU: ${item.skuDescricao || item.skuCodigo}` : ''}`, 
        ...
        const condicoes = [
            ...
            `- Taxa financeira inclusa: ${orcamento.taxaFinanceiraPercentual || 0}%.`,
