...
export const erpSimulations = pgTable(
  'erp_simulations',
  {
...
export const configuracoesPrecificacao = pgTable('configuracoes_precificacao', {
  ...
  moProducaoPctPadrao: numeric('mo_producao_pct_padrao', { precision: 5, scale: 4 }).default(
    '0.30',
  ),
  moInstalacaoPctPadrao: numeric('mo_instalacao_pct_padrao', { precision: 5, scale: 4 }).default(
    '0.15',
  ),
  margemMinimaAlerta: numeric('margem_minima_alerta', { precision: 5, scale: 4 }).default('0.25'),
  atualizadoEm: timestamp('atualizado_em', { withTimezone: true, mode: 'string' }).default(
    sql`CURRENT_TIMESTAMP`,
  ),
...
export const categoriasMaterial = pgTable(
  'categorias_material',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    nome: text().notNull(),
    slug: text().notNull(),
    icone: text(),
    criadoEm: timestamp('criado_em', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
    ),
  },
  (table) => [unique('categorias_material_slug_key').on(table.slug)],
);
...
export const itensOrcamento = pgTable(
  'itens_orcamento',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const historicoChamado = pgTable(
  'historico_chamado',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const clients = pgTable(
  ...
  (table) => [
    uniqueIndex('clients_tenant_cnpj_idx')
      .using(
        'btree',
        table.tenantId.asc().nullsLast().op('text_ops'),
        table.cnpj.asc().nullsLast().op('uuid_ops'),
      )
      .where(sql`(cnpj IS NOT NULL)`),
    uniqueIndex('clients_tenant_cpf_idx')
      .using(
        'btree',
        table.tenantId.asc().nullsLast().op('text_ops'),
        table.cpf.asc().nullsLast().op('uuid_ops'),
      )
...
export const skuComponente = pgTable(
  'sku_componente',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [unique('sku_componente_codigo_key').on(table.codigo)],
);

export const kanbanItems = pgTable(
  'kanban_items',
  {
    id: serial().primaryKey().notNull(),
    title: text().notNull(),
...
export const erpSkus = pgTable(
  ...
  (table) => [unique('erp_skus_sku_code_key').on(table.skuCode)],
);

export const erpProductBom = pgTable(
  'erp_product_bom',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    ...
    precoMaterialM3Padrao: numeric('preco_material_m3_padrao', { precision: 12, scale: 2 }).default(
      '0',
    ),
...
export const projetoTipos = pgTable(
  'projeto_tipos',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const erpFamilies = pgTable(
  'erp_families',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    nome: text().notNull(),
...
export const erpSubfamilies = pgTable(
  'erp_subfamilies',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    nome: text().notNull(),
...
export const planosDeCorte = pgTable(
  'planos_de_corte',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const erpProjectItems = pgTable(
  'erp_project_items',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const erpConsumptionResults = pgTable(
  'erp_consumption_results',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const erpSkusEngenharia = pgTable(
  'erp_skus_engenharia',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const classesFinanceiras = pgTable(
  'classes_financeiras',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index('idx_classes_financeiras_codigo').using(
      'btree',
...
export const pedidoCompraItens = pgTable(
  'pedido_compra_itens',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const contasInternas = pgTable(
  'contas_internas',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    nome: varchar({ length: 255 }).notNull(),
...
export const retalhosEstoque = pgTable(
  'retalhos_estoque',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index('idx_retalhos_area')
      .using('btree', sql`((largura_mm * altura_mm))`)
      .where(sql`((disponivel = true) AND (descartado = false))`),
    index('idx_retalhos_criado')
      .using('btree', table.createdAt.asc().nullsLast().op('timestamptz_ops'))
      .where(sql`((disponivel = true) AND (descartado = false))`),
    index('idx_retalhos_disponivel_sku')
      .using(
        'btree',
        table.skuChapa.asc().nullsLast().op('text_ops'),
        table.disponivel.asc().nullsLast().op('bool_ops'),
        table.descartado.asc().nullsLast().op('text_ops'),
      )
      .where(sql`((disponivel = true) AND (descartado = false))`),
    index('idx_retalhos_origem')
      .using('btree', table.planoCorteOrigemId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(plano_corte_origem_id IS NOT NULL)`),
    index('idx_retalhos_otimizacao')
      .using('btree', sql`sku_chapa`, sql`((largura_mm * altura_mm))`, sql`created_at`)
      .where(sql`((disponivel = true) AND (descartado = false))`),
    index('idx_retalhos_tamanho')
      .using(
        'btree',
        table.larguraMm.asc().nullsLast().op('int4_ops'),
        table.alturaMm.asc().nullsLast().op('int4_ops'),
      )
      .where(sql`((disponivel = true) AND (descartado = false))`),
    index('idx_retalhos_utilizacao')
      .using('btree', table.utilizadoEmId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(utilizado_em_id IS NOT NULL)`),
    foreignKey({
      columns: [table.tenantId],
      foreignColumns: [tenants.id],
    ...
    check(
      'retalhos_estoque_origem_check',
      sql`(origem)::text = ANY ((ARRAY['sobra_plano_corte'::character varying, 'devolucao'::character varying, 'manual'::character varying, 'ajuste'::character varying])::text[])`,
    ),
  ],
);

export const contasRecorrentes = pgTable(
  'contas_recorrentes',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const titulosReceber = pgTable(
  'titulos_receber',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    ...
    valorCustoFinanceiro: numeric('valor_custo_financeiro', { precision: 15, scale: 2 }).default(
      '0',
    ),
    tenantId: uuid('tenant_id'),
  },
  (table) => [
    index('idx_titulos_receber_status_deletado').using(
      'btree',
      table.status.asc().nullsLast().op('bool_ops'),
      table.deletado.asc().nullsLast().op('bool_ops'),
    ),
    index('idx_titulos_receber_vencimento').using(
      'btree',
      table.dataVencimento.asc().nullsLast().op('timestamp_ops'),
    ),
    foreignKey({
      columns: [table.condicaoPagamentoId],
      foreignColumns: [condicoesPagamento.id],
...
export const titulosPagar = pgTable(
  'titulos_pagar',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    numeroTitulo: varchar('numero_titulo', { length: 50 }).notNull(),
  ...
  (table) => [
    index('idx_titulos_pagar_status_deletado').using(
      'btree',
      table.status.asc().nullsLast().op('bool_ops'),
      table.deletado.asc().nullsLast().op('bool_ops'),
    ),
    index('idx_titulos_pagar_vencimento').using(
      'btree',
      table.dataVencimento.asc().nullsLast().op('timestamp_ops'),
    ),
    foreignKey({
...
export const condicoesPagamento = pgTable(
  'condicoes_pagamento',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    nome: text().notNull(),
...
export const skuEngenharia = pgTable(
  'sku_engenharia',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    codigo: varchar({ length: 20 }).notNull(),
  ...
  (table) => [unique('sku_engenharia_codigo_key').on(table.codigo)],
);
...
export const erpChapas = pgTable(
  'erp_chapas',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    sku: varchar({ length: 100 }).notNull(),
...
export const skuMontagem = pgTable(
  'sku_montagem',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    codigo: varchar({ length: 20 }).notNull(),
  ...
  (table) => [unique('sku_montagem_codigo_key').on(table.codigo)],
);
...
export const bomEngenhariaMontagem = pgTable(
  ...
  (table) => [
    ...
    unique('bom_engenharia_montagem_sku_engenharia_id_sku_montagem_id_key').on(
      table.skuEngenhariaId,
      table.skuMontagemId,
    ),
  ],
);

export const erpMovimentacoesIndustrial = pgTable(
...
export const eventos = pgTable(
  ...
  (table) => [
    index('idx_eventos_cliente')
      .using('btree', table.clienteId.asc().nullsLast().op('text_ops'))
      .where(sql`(cliente_id IS NOT NULL)`),
    index('idx_eventos_data_inicio').using(
      'btree',
      table.dataInicio.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_eventos_responsavel').using(
      'btree',
      table.responsavelId.asc().nullsLast().op('text_ops'),
    ),
    index('idx_eventos_status_visita')
      .using('btree', table.statusVisita.asc().nullsLast().op('text_ops'))
      .where(sql`((tipo)::text = 'visita'::text)`),
    ...
    check(
      'eventos_tipo_check',
      sql`(tipo)::text = ANY ((ARRAY['visita'::character varying, 'reuniao'::character varying, 'compromisso'::character varying, 'deadline'::character varying, 'outro'::character varying])::text[])`,
    ),
    check(
      'eventos_status_visita_check',
      sql`(status_visita)::text = ANY ((ARRAY['agendado'::character varying, 'realizado'::character varying, 'follow_up'::character varying, 'cancelado'::character varying])::text[])`,
    ),
    check(
      'visita_requer_status',
      sql`((tipo)::text <> 'visita'::text) OR (status_visita IS NOT NULL)`,
    ),
    check('data_fim_depois_inicio', sql`data_fim > data_inicio`),
    check(
      'visita_requer_cliente',
      sql`((tipo)::text <> 'visita'::text) OR (cliente_id IS NOT NULL)`,
    ),
  ],
);

export const bomMontagemComponente = pgTable(
  ...
  (table) => [
    ...
    unique('bom_montagem_componente_sku_montagem_id_sku_componente_id_key').on(
      table.skuMontagemId,
      table.skuComponenteId,
    ),
  ],
);
...
export const ordensProducao = pgTable(
  ...
  (table) => [
    index('idx_ordens_producao_projeto_id').using(
      'btree',
      table.projetoId.asc().nullsLast().op('text_ops'),
    ),
    index('idx_ordens_producao_status').using(
      'btree',
...
export const conhecimentoMarcenaria = pgTable(
  ...
  (table) => [
    index('conhecimento_marcenaria_embedding_idx').using(
      'hnsw',
...
export const usageLogs = pgTable(
  'usage_logs',
  {
    ...
    custoEstimado: numeric('custo_estimado', { precision: 15, scale: 8 })
      .default('0.00000000')
      .notNull(),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' })
      .default(sql`CURRENT_TIMESTAMP`)
...
export const chamadosGarantia = pgTable(
  'chamados_garantia',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    projetoId: text('projeto_id'),
...
export const erpCategories = pgTable(
  'erp_categories',
  {
...
export const conversasWhatsapp = pgTable(
  'conversas_whatsapp',
  {
    ...
    dataCriacao: timestamp('data_criacao', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
    ),
    updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
...
export const etapasProdKanban = pgTable(
  'etapas_prod_kanban',
  {
    id: serial().primaryKey().notNull(),
    operacaoProdId: uuid('operacao_prod_id'),
...
export const movimentoKanban = pgTable(
  'movimento_kanban',
  {
    id: serial().primaryKey().notNull(),
    ...
    timestampMovimento: timestamp('timestamp_movimento', {
      withTimezone: true,
      mode: 'string',
    }).default(sql`CURRENT_TIMESTAMP`),
    nota: text(),
...
export const notificacoesCalendario = pgTable(
  'notificacoes_calendario',
  {
    ...
    mensagem: text(),
    enviadoEm: timestamp('enviado_em', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
    ),
...
export const ordensProd = pgTable(
  'ordens_prod',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    orcamentoId: uuid('orcamento_id'),
...
export const custosReaisOp = pgTable(
  'custos_reais_op',
  {
    id: serial().primaryKey().notNull(),
    ...
    custoDesperdicioMaterial: numeric('custo_desperdicio_material', {
      precision: 10,
      scale: 2,
    }).default('0'),
...
export const tendenciasPreco = pgTable(
  'tendencias_preco',
  {
    id: serial().primaryKey().notNull(),
...
export const rentabilidadeCliente = pgTable(
  'rentabilidade_cliente',
  {
    ...
    margemMediaPercentual: numeric('margem_media_percentual', { precision: 5, scale: 2 }).default(
      '0',
    ),
...
export const mensagensWhatsapp = pgTable(
  'mensagens_whatsapp',
  {
    id: serial().primaryKey().notNull(),
    ...
    timestampMsg: timestamp('timestamp_msg', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
    ),
    lido: boolean().default(false),
...
export const modelosMsgWhatsapp = pgTable(
...
export const alertasEstoque = pgTable(
  'alertas_estoque',
  {
    ...
    ativo: boolean().default(true),
    dataAlerta: timestamp('data_alerta', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
    ),
    dataResolucao: timestamp('data_resolucao', { withTimezone: true, mode: 'string' }),
...
export const contratoDigital = pgTable(
  'contrato_digital',
  {
    id: serial().primaryKey().notNull(),
    orcamentoId: uuid('orcamento_id'),
    ...
    dataSolicitacaoAssinatura: timestamp('data_solicitacao_assinatura', {
      withTimezone: true,
      mode: 'string',
    }),
    idAssinaturaExterna: varchar('id_assinatura_externa', { length: 100 }),
    urlAssinatura: varchar('url_assinatura', { length: 500 }),
    dataAssinaturaEmpresa: timestamp('data_assinatura_empresa', {
      withTimezone: true,
      mode: 'string',
    }),
    dataAssinaturaCliente: timestamp('data_assinatura_cliente', {
      withTimezone: true,
      mode: 'string',
    }),
...
export const estoqueMateriaisDetalhado = pgTable(
  'estoque_materiais_detalhado',
  {
    ...
    dataProximaReposicao: timestamp('data_proxima_reposicao', {
      withTimezone: true,
      mode: 'string',
    }),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' }).default(
...
export const historicoAssinaturaDigital = pgTable(
  'historico_assinatura_digital',
  {
    id: serial().primaryKey().notNull(),
    contratoId: integer('contrato_id').notNull(),
    acao: varchar({ length: 100 }).notNull(),
    timestampAcao: timestamp('timestamp_acao', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
    ),
    usuarioId: uuid('usuario_id'),
    detalhes: text(),
...
export const historicoSkuMatching = pgTable(
  'historico_sku_matching',
  {
    ...
    timestampMatching: timestamp('timestamp_matching', {
      withTimezone: true,
      mode: 'string',
    }).default(sql`CURRENT_TIMESTAMP`),
    usuarioId: uuid('usuario_id'),
...
export const quotations = pgTable(
  'quotations',
  {
    ...
    taxaFinanceiraPercentual: numeric('taxa_financeira_percentual', {
      precision: 5,
      scale: 2,
    }).default('0'),
  ...
  (table) => [
    index('quotations_cliente_id_idx').using(
      'btree',
      table.clienteId.asc().nullsLast().op('int4_ops'),
    ),
    index('quotations_data_orcamento_idx').using(
      'btree',
      table.dataOrcamento.asc().nullsLast().op('timestamp_ops'),
    ),
...
export const quotationItems = pgTable(
  ...
  (table) => [
    index('quotation_items_orcamento_id_idx').using(
      'btree',
      table.quotationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('quotation_items_orcamento_id_idx1').using(
      'btree',
      table.quotationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('quotation_items_orcamento_id_idx2').using(
      'btree',
      table.quotationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('quotation_items_sku_codigo_idx').using(
      'btree',
      table.skuCodigo.asc().nullsLast().op('text_ops'),
    ),
    index('quotation_items_sku_componente_id_idx').using(
      'btree',
      table.skuComponenteId.asc().nullsLast().op('uuid_ops'),
    ),
    index('quotation_items_sku_componente_id_idx1').using(
      'btree',
      table.skuComponenteId.asc().nullsLast().op('uuid_ops'),
    ),
    index('quotation_items_sku_engenharia_id_idx').using(
      'btree',
      table.skuEngenhariaId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
// REMOVED in 2026-06-05: quotationBom (duplicate of src/db/schema/quotations.ts:86)
// REMOVED in 2026-06-05: orcamentos table — never used by runtime (consolidated into `quotations`)
// REMOVED in 2026-06-05: orcamentoAmbientes, orcamentoMoveis, orcamentoPecas, orcamentoFerragens, orcamentoCustosExtras — all dropped from DB

export const prospeccoes = pgTable(
  'prospeccoes',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    tenantId: uuid('tenant_id'),
  ...
  (table) => [
    index('prospeccoes_status_idx').using(
      'btree',
      table.tenantId.asc().nullsLast().op('text_ops'),
      table.status.asc().nullsLast().op('text_ops'),
    ),
...
export const interacoesProspeccao = pgTable(
  'interacoes_prospeccao',
  {
    ...
    dataInteracao: timestamp('data_interacao', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
    ),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' }).default(
      sql`CURRENT_TIMESTAMP`,
  ...
  (table) => [
    index('interacoes_prosp_idx').using(
      'btree',
      table.prospeccaoId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const retalhosDisponiveis = pgView('retalhos_disponiveis', {
  id: uuid(),
